...
CREATE PROCEDURE pETL_Desastres()
	DELETE FROM desastres_bde.desastres_final;
	INSERT INTO desastres_bde.desastres_final (Cuatrenio, Temp_AVG, Oxi_AVG, T_Tsunamis, T_OlasCalor, T_Terremotos, T_Erupciones, T_Incendios, M_Jovenes_AVG, M_Adutos_AVG, M_Ancianos_AVG)
		SELECT
			CASE WHEN d.año <= 2026 THEN '2023-2026' ELSE '2027-2030' END as Cuatrenio,
			ROUND(AVG(c.Temperatura)),
            ROUND(AVG(c.Oxigeno)),
			SUM(d.Tsunamis),
            SUM(d.Olas_Calor),
            SUM(d.Terremotos),
            SUM(d.Erupciones),
            SUM(d.Incendios),
            ROUND(AVG(m.R_Menor15 + m.R_15_a_30)),
            ROUND(AVG(R_30_a_45 + R_45_a_60)),
            ROUND(AVG(m.R_M_a_60))

		FROM DESASTRES.desastres d LEFT JOIN DESASTRES.clima c ON d.año = c.año LEFT JOIN DESASTRES.muertes m ON c.año = m.año
	GROUP BY Cuatrenio
;
