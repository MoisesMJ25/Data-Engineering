-- Create new table in schema Data_engineer
CREATE TABLE data_engineer.eventos_apocalipticos (
	id_evento INT,
...
INSERT INTO data_engineer.eventos_apocalipticos (id_evento, nombre_evento, fecha_evento, descripcion_evento)
VALUES
...
-- Create new table
CREATE TABLE data_engineer.prediccion_fin_mundo (
	id_evento int,
    nombre_evento varchar(100),
    fecha_evento date,
    dias_faltantes int,
    fuente_prediccion varchar(30)
)
;

-- Create procedure 'pETL' to insert new information from other table
USE data_engineer; 
CREATE PROCEDURE pETL()

	DELETE FROM data_engineer.prediccion_fin_mundo;
	INSERT INTO data_engineer.prediccion_fin_mundo (id_evento, nombre_evento, fecha_evento, dias_faltantes, fuente_prediccion)

		SELECT
			id_evento,
            nombre_evento,
            fecha_evento,
            datediff(fecha_evento, now()) dias_faltantes,
            'NERT' fuente_prediccion
		FROM data_engineer.eventos_apocalipticos
;
